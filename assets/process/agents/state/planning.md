...
2. Draft the plan yourself, directly with `tod-cli plan` (see **Plan steps** below) — before considering whether to open an interview. Most of a plan follows mechanically from settled design obligations; generate everything you can determine on your own right now, on this transition, rather than leaving it for interview turns to build up piecemeal.
3. If plan steps exist and cover everything you can determine, and gate criteria pass → verify conformance and traceability, then proceed to exit.
