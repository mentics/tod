...
pub(super) fn commit_worktree(cwd: &std::path::Path, branch: &str, message: &str) -> Result<bool> {
    ...
    let branch = current_branch(cwd).unwrap_or_else(|| branch.to_string());
    let submodules = worktree::submodule_dirs(cwd).unwrap_or_default();
    let mut committed = false;
    for dir in submodules.iter().rev() {
        committed |= commit_repo(dir, &branch, message)?;
    }
    worktree::prune_submodule_branches(cwd, &branch)?;
    Ok(commit_repo(cwd, &branch, message)? || committed)
}

/// Commit everything in the one repository at `cwd`; see [`commit_worktree`].
fn commit_repo(cwd: &std::path::Path, branch: &str, message: &str) -> Result<bool> {
