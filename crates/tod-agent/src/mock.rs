...
use crate::util::{new_transcript_filename, path_for_storage};
...
impl AgentProvider for MockAgentProvider {
    ...
    fn start_deep_dive_chat(
        ...
        // The caller assembles the deep-dive prompt (role doc + question
        // context); the mock only echoes a bounded slice of it back.
        let reply = format!(
            "Mock deep-dive reply:{}Consider: {}",
            "\n\n",
...
mod tests {
    ...
    /// Mirrors `tod_core::interview::config::agent_scratchpad_for_node`.
    /// Duplicated in test code only, so this crate stays dependency-free.
    fn agent_scratchpad_for_node(data_root: &std::path::Path, node_id: uuid::Uuid) -> PathBuf {
        data_root
            .join("agent")
            .join("nodes")
    ...
    fn question_maker_prompt(turn: &str) -> AgentPrompt {
        AgentPrompt {
