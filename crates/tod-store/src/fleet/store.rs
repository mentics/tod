...
impl FleetStore {
    ...
    /// "Implement" button) for a config, newest first.
    pub fn list_implementation_sessions_for_config(&self, config_id: &str) -> Result<Vec<AgentRun>> {
        let guard = self.projection.lock().expect("fleet projection mutex");
        AgentRunRepo::new(&guard.connection())
    ...
    /// The live implementation-kind run for a config, if any — backs the
    /// lifecycle panel's one-at-a-time Implement lock.
    pub fn live_implementation_session_for_config(&self, config_id: &str) -> Result<Option<AgentRun>> {
        let guard = self.projection.lock().expect("fleet projection mutex");
        AgentRunRepo::new(&guard.connection())
            .has_live_implementation_run(config_id)
            .map_err(Into::into)
    }
