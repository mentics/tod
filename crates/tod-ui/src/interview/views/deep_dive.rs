...
/// Fallback when the bundled deep-dive role doc cannot be resolved.
const DEFAULT_DEEP_DIVE_ROLE: &str = "# Deep dive";
...
impl DeepDiveView {
    ...
    fn send_message(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        // Prompt assembly is a core concern: the agent crate is handed a
        // finished string and never reaches into the process bundle itself.
        let context = self.deep_dive_context();
        let role_doc = tod_core::process_bundle::TodInstallPaths::discover()
            .ok()
            .and_then(|install| tod_core::process_bundle::load_deep_dive_role_doc(&install).ok())
            .unwrap_or_else(|| DEFAULT_DEEP_DIVE_ROLE.to_string());
        let conversation = conversation_transcript(&self.turns);
        let cwd = self.workspace_cwd.clone();
        let agent = self.agent.clone();
        let prompt = tod_core::process_bundle::build_deep_dive_prompt(
            &role_doc,
            &context,
            Some(&conversation),
        );
                        ...
                        v_flex()
                            ...
                            .children(self.turns.iter().enumerate().map(|(idx, turn)| {
                                ...
                                div()
                                    ...
                                    .child(
                                        div().text_sm().text_color(foreground).child(
                                            crate::ui::selectable_text::selectable_markdown(
                                                ("deep-dive-turn-body", idx),
                                                turn.body.clone(),
                                                window,
                                                cx,
                                            )
