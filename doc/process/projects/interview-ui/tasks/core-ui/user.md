...
1. Full project scope — Deliver all requirements numbered 1–24 in `doc/process/projects/interview-ui/user.md` (including unbootstrapped session recovery, selection visibility, single-line list rows, workspace keyboard/pointer navigation, Escape, MC digit submit, non-blocking answer submit, response column layout, and Complete-only-when-queue-empty). The task is complete only when each passes its statement and success-criteria checks (where present).
